...
const normalizeHistoryResponse = (response) => {
	const data = response?.data || response?.results || response?.readings || response;
...
const getReadingValue = (item) => {
	return (
		item?.value ??
		item?.hourmeter ??
		item?.hourmeter_value ??
		item?.current_hourmeter ??
		null
	);
};

const getReadingDate = (item) => {
	return (
		item?.measured_at ||
		item?.created_at ||
		item?.date ||
...
const getSourceLabel = (source) => {
	if (source === "app") return "Aplicativo";
	if (source === "app_offline") return "App offline";
...
const ReadingCard = ({ item, previousValue }) => {
	const value = getReadingValue(item);
	const date = getReadingDate(item);
		...
		<View style={styles.historyCard}>
			<View style={styles.historyTopLine}>
				...
				<View style={styles.historyTextBox}>
					<Text style={styles.historyTitle}>Leitura de horímetro</Text>
					<Text style={styles.historyDate}>{formatDateTimeBR(date)}</Text>
				</View>

				<View style={styles.valueBox}>
					<Text style={styles.valueLabel}>Leitura</Text>
					<Text style={styles.valueText}>{formatHour(value)} h</Text>
				</View>
			</View>
			...
			<View style={styles.historyMetaRow}>
				<View style={styles.metaPill}>
					...
					<Text style={styles.metaPillText}>{source}</Text>
				</View>

				{difference !== null && !Number.isNaN(difference) ? (
					<View style={styles.metaPill}>
						<Ionicons name="trending-up-outline" size={13} color="rgba(15,23,42,0.48)" />
						<Text style={styles.metaPillText}>
							+{formatHour(Math.max(0, difference))} h
						</Text>
					</View>
				) : null}
			</View>
	...
	return (
		<View style={styles.emptyCard}>
			<View style={styles.emptyIconBox}>
				<Ionicons name="speedometer-outline" size={25} color={Colors.primary[800]} />
			</View>

			<Text style={styles.emptyTitle}>Nenhuma leitura encontrada</Text>
			<Text style={styles.emptyDescription}>
				As leituras de horímetro registradas para esta máquina aparecerão aqui.
			</Text>
		</View>
	);
};

const MachineReadingsHistoryScreen = ({ route, navigation }) => {
	...
	const sortedReadings = useMemo(() => {
		return [...readings].sort((a, b) => {
			const dateA = new Date(getReadingDate(a) || 0).getTime();
			const dateB = new Date(getReadingDate(b) || 0).getTime();
		...
		async ({ refreshing = false } = {}) => {
			...
			try {
				...
				const response = await fetch(
					`${LINKMachine}/maquinario/machines/${machine.id}/readings/`,
					{
						method: "GET",
						headers: {
				...
				if (!response.ok) {
					...
					throw new Error(text || "Erro ao carregar leituras.");
				}

				const data = await response.json();
				setReadings(normalizeHistoryResponse(data));
			} catch (err) {
				console.log("Erro ao carregar leituras:", err);
				setError(err?.message || "Não foi possível carregar as leituras.");
			} finally {
				setIsLoading(false);
			...
			<ScrollView
				...
				refreshControl={
					<RefreshControl
						...
						onRefresh={() => loadReadings({ refreshing: true })}
			...
			>
				<View style={styles.topBar}>
					...
					<View style={styles.topTitleBox}>
						<Text style={styles.topTitle}>Histórico de leituras</Text>
						<Text style={styles.topSubtitle}>Registros de horímetro</Text>
					</View>
				</View>
				...
				<View style={styles.sectionHeader}>
					<View>
						...
						<Text style={styles.sectionDescription}>
							{sortedReadings.length} registro{sortedReadings.length === 1 ? "" : "s"}
						</Text>
					</View>

					<Pressable
						onPress={() => loadReadings({ refreshing: true })}
						style={({ pressed }) => [
				...
				{isLoading ? (
					...
					sortedReadings.map((item, index) => {
						const nextItem = sortedReadings[index + 1];
						const previousValue = nextItem ? getReadingValue(nextItem) : null;

						return (
							<ReadingCard
								key={`${item?.id || "reading"}-${index}`}
								item={item}
...
export default MachineReadingsHistoryScreen;