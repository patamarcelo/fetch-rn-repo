...
function getMapSvgString(highlightIds, rawPolygons, culture) {
    try {
...
export const createApplicationPdfMap = async (data, farm, plotMap) => {
    ...
    const apCotainer = data.filter((op) => !op.operation.toLowerCase().includes('colheita')).map((app) => {
        ...
        console.log('[PDF] variaveis – talhoesParaPintar', talhoesParaPintar);
        console.log('[PDF] variaveis – dataFromJson', dataFromJson.length);
        console.log('[PDF] variaveis – cultura filter color', cultura);
        console.time('[PDF] gerar-SVGs');
        let svgDataUri = ''
        let svgMinified = ""
        try {
            svgMinified = getMapSvgString(talhoesParaPintar, dataFromJson, cultura);
            console.log('[PDF] passo 2 finalizado, formatando agora para uri');
            svgDataUri  = svgToDataUri(svgMinified);   
        } catch (error) {
            console.log('[PDF] erro ao gerar os mapas para o PDF ', error);
        }
